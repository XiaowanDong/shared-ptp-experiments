...
ssh root@10.79.20.116 << EOF
	...
	cpuset -c -l 7 ./run_compiler_nosuperpg_promo.sh L_J_nosuperpg >& results/out.txt
	mv results/out.txt results/java_ld_compiler_500ops_L_J_nosuperpg_usr_2/
	reboot -k kernel_all
EOF
sleep 180
...
ssh root@10.79.20.116 << EOF
	...
	cd /home/xd10/tests/java
	cpuset -c -l 2,3,4,5 java -XX:+AlwaysPreTouch -jar SPECjvm2008.jar -ikv -ict -wt 0 -crf false -ops 500 -Dspecjvm.hardware.threads.override=4 compiler.compiler
	cpuset -c -l 7 ./run_compiler_nosuperpg_promo.sh Ls_Js_data_nosuperpg >& results/out.txt
	mv results/out.txt results/java_ld_compiler_500ops_Ls_Js_data_nosuperpg_usr_2/
...
ssh root@10.79.20.116 << EOF
	...
	cpuset -c -l 7 ./run_compiler.sh L-s_J-s_data_superpg >& results/out.txt
	mv results/out.txt results/java_ld_compiler_500ops_L-s_J-s_data_superpg_usr_2/
	reboot -k agg-data-superpg
EOF
sleep 180
...
ssh root@10.79.20.116 << EOF
	...
	cpuset -c -l 2,3,4,5 java -XX:+AlwaysPreTouch -XX:CodeCacheExpansionSize=2m -jar SPECjvm2008.jar -ikv -ict -wt 0 -crf false -ops 500 -Dspecjvm.hardware.threads.override=4 compiler.compiler
	dd if=/usr/local/bootstrap-openjdk8/jre/lib/amd64/server/libjvm.so of=/dev/null
	cpuset -c -l 7 ./run_compiler.sh Ls_Js_data_superpg >& results/out.txt
	mv results/out.txt results/java_ld_compiler_500ops_Ls_Js_data_superpg_usr_2/
	reboot -k kernel_all
EOF
sleep 180
