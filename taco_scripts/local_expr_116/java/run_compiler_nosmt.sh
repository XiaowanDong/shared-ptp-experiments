...
while [ $ITER -lt $TOTAL_ITERS ]; do
	./get_compiler_procstat_nosmt.sh > "$RESULTS_DIR"/procstat_"$TEST_NAME"_"$ITER"_before.txt 2>&1
...
#               -s ITLB.ITLB_FLUSH,usr" \
	...
	do
		((NUM++))
		./bench_java_compiler_nosmt.sh "$EVENTS" $NUM $TEST_NAME $ITER
	done

	./get_compiler_procstat_nosmt.sh > "$RESULTS_DIR"/procstat_"$TEST_NAME"_"$ITER"_after.txt 2>&1
